...
//! Electron app's APP_ROOT/../.. trick, because StockSense-Setup.exe is meant
...
///
/// `pin` is the commit-or-branch from either StockSense Setup's build-time
...
async fn download(kind: ScriptKind, commit_or_ref: &str, dest_path: &Path) -> Result<()> {
    let url = format!(
        "https://raw.githubusercontent.com/jiapeng-github/hermes-agent/{}/scripts/{}",
        commit_or_ref,
        kind.filename()
    );
    ...
    let response = reqwest::Client::builder()
        ...
        .get(&url)
        .header("User-Agent", "stocksense-setup/0.19.0")
        .send()
        .await
