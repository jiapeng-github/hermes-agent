#!/usr/bin/env node
// set-exe-identity.mjs — stamp the StockSense icon + version metadata onto the
// built StockSense.exe using rcedit, completely decoupled from electron-builder's
// signing path.
//
// WHY THIS EXISTS
// ---------------
...
async function stampExeIdentity(exe, desktopRoot = resolve(import.meta.dirname, '..')) {
  ...
  await rcedit(exe, {
    icon,
    'version-string': {
      ProductName: 'Finance Mate',
      FileDescription: 'Finance Mate',
      CompanyName: 'Nous Research',
      LegalCopyright: 'Copyright (c) 2026 Nous Research'
    }
  })

  console.log('[set-exe-identity] done — StockSense icon + identity stamped')
}
