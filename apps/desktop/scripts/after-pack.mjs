 ...
 *
 * Stamps the StockSense icon + identity onto the packed Windows StockSense.exe via
...
export default async function afterPack(context) {
  ...
  const productName = context.packager?.appInfo?.productFilename || 'StockSense'
  ...
  try {
    ...
    console.warn(`[after-pack] exe identity stamp failed (${err.message}); StockSense.exe keeps the stock Electron icon`)
  }
}
