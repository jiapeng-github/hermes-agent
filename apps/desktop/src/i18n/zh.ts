...
export const zh: Translations = {
  ...
  boot: {
    ...
    steps: {
      ...
      loadingSettings: '正在加载 天机设置',
      loadingSessions: '正在加载最近会话',
      startingDesktopConnection: '正在启动桌面连接',
      startingHermesDesktop: '正在启动 天机桌面版…'
    },
    errors: {
      backgroundExited: '天机后台进程已退出。',
      backgroundExitedDuringStartup: '天机后台进程在启动期间退出。',
  ...
  notifications: {
    ...
    backendOutOfDateMessage: '你的 天机后端早于当前桌面构建，可能无法正常工作。请更新以保持一致。',
    ...
    errors: {
      ...
      methodNotAllowed: '桌面后端拒绝了该请求 (405 Method Not Allowed)。请尝试重启 天机桌面版。',
  ...
  settings: {
    ...
    notifications: {
      ...
      kinds: {
        ...
        input: {
          label: '需要输入',
          description: '天机提出了问题，或需要密码或密钥。'
        },
        turnDone: {
          label: '回复就绪',
          description: '天机在后台时完成了一轮对话。'
        },
        turnError: {
          label: '本轮失败',
          description: '本轮以错误结束。'
    ...
    appearance: {
      ...
      colorMode: '颜色模式',
      colorModeDesc: '选择固定模式，或让 天机跟随系统设置。',
      ...
      pet: {
        title: '宠物',
        intro:
          '领养一只悬浮在应用上的 petdex 动画宠物，它会根据 天机的状态做出反应——工具执行时奔跑、成功时欢呼、出错时沮丧。',
    ...
    fieldDescriptions: defineFieldCopy({
      ...
      timezone: '当 天机需要本地时间上下文时使用。留空则使用系统时区。',
      agent: {
        imageInputMode: '控制图片附件如何发送给模型。',
        maxTurns: '天机停止一次运行前工具调用轮次的上限。'
      },
      terminal: {
        cwd: '工具与终端操作的默认项目目录。',
      ...
      fileReadMaxChars: '天机单次文件读取可读取的最大字符数。',
      approvals: {
      ...
      updates: {
        nonInteractiveLocalChanges:
          '天机从应用内更新时（无终端提示），保留本地源码修改（暂存）或丢弃（放弃）。通过终端更新时始终会询问。'
      }
    }),
    about: {
      heading: '天机桌面版',
      ...
      automaticUpdatesDesc: '天机会在后台自动检查更新，并在有可用更新时通知你。',
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        '天机桌面版 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 天机后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
      ...
      localTitle: '本地网关',
      localDesc: '在 localhost 启动私有 天机后端。这是默认方式，并且可离线工作。',
      remoteTitle: '远程网关',
      remoteDesc:
        '将此桌面外壳连接到远程 天机后端。托管网关使用 OAuth 或用户名密码；自托管网关也可能使用会话 token。',
      ...
      savedTitle: '网关设置已保存',
      restartingMessage: '天机桌面版 将使用已保存设置重新连接。',
    ...
    providers: {
      ...
      intro: '使用订阅登录，无需复制 API 密钥。天机会在应用中为你完成浏览器登录。',
      connected: '已连接',
      collapse: '收起',
  ...
  starmap: {
    ...
    loading: '加载中…',
    emptyTitle: '尚无学习内容',
    emptyDesc: '当 天机为你的工作构建技能和记忆时，会显示在这里。',
    share: '分享图谱',
  ...
  commandCenter: {
    ...
    pets: {
      ...
      staleBackend: '请重启天机 以使用宠物功能——当前后端版本过旧。',
      empty: '没有匹配的宠物。',
      turnOff: '关闭',
      turnOn: '开启',
      installed: '已安装',
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      weixin:
        '运行 `hermes gateway setup`，选择 Weixin，然后使用个人微信账号扫描并确认二维码。天机会通过腾讯 iLink Bot API 连接并保存凭据。',
      qqbot: '在 QQ 开放平台 (q.qq.com) 注册一个应用，复制 App ID 和 Client Secret。',
      api_server:
        '把 天机暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  profiles: {
    ...
    deleting: '删除中…',
    createDesc: '配置档案是相互独立的 天机环境：各自拥有独立的配置、技能和 SOUL.md。',
    nameLabel: '名称',
    cloneFrom: '克隆来源',
    cloneFromNone: '无（空白）',
  ...
  sidebar: {
    ...
    projects: {
      ...
      createFailed: '无法创建项目',
      staleBackend: '请更新 天机后端以创建项目——当前后端比桌面应用旧（设置 → 更新 → 后端）。',
  ...
  composer: {
    ...
    attachUrlDesc: '天机将抓取该页面并作为本回合的上下文。',
  ...
  updates: {
    ...
    availableBodyBackend: '已连接的 天机后端有新版本可安装。',
  ...
  onboarding: {
    ...
    recommended: '推荐',
    connected: '已连接',
    featuredPitch: '一个订阅，300+ 前沿模型 — 运行天机 的推荐方式',
    ...
    authorizeThere: '请在那里授权天机。',
    copyAuthCode: '复制授权码并粘贴到下面。',
    pasteAuthCode: '粘贴授权码',
    reopenAuthPage: '重新打开授权页面',
    autoBrowser: provider => `已在浏览器中打开 ${provider}。请在那里授权天机，连接会自动完成，无需复制或粘贴。`,
  ...
  shell: {
    ...
    statusbar: {
      ...
      desktopVersion: version => `天机桌面版 v${version}`,
  ...
  preview: {
    ...
    web: {
      ...
      finishedRestarting: message => `天机已完成预览服务器重启${message ? `: ${message}` : ''}`,
  ...
  prompts: {
    ...
    sudoTitle: '管理员密码',
    sudoDesc: '天机需要你的 sudo 密码来运行特权命令。它只会发送给你的本地 agent。',
    sudoPlaceholder: 'sudo 密码',
    secretTitle: '需要密钥',
    secretDesc: '天机需要一个凭据才能继续。',
  ...
  desktop: {
    ...
    restartToUseSaveImage: '重启 天机桌面版后可使用保存图片。',
    restartToSaveImages: '重启 天机桌面版以保存图片',
