...
export const zh: Translations = {
  ...
  settings: {
    ...
    gateway: {
      ...
      envOverride: '环境变量覆盖',
      intro:
        '金融助手 Desktop 默认会启动自己的本地网关。当你希望此应用控制另一台机器上或可信代理后的现有 金融助手 后端时，可以使用远程网关。下面可按 profile 指定各自的远程主机。',
      appliesTo: '应用于',
  ...
  messaging: {
    ...
    platformIntro: {
      ...
      bluebubbles: '在装有 iMessage 的 Mac 上运行 BlueBubbles Server，暴露其 API，然后用服务器密码把 金融助手 指向该 URL。',
      ...
      api_server: '把 金融助手 暴露为兼容 OpenAI 的 API。设置一个鉴权密钥，然后把 Open WebUI / LobeChat 等指向 host:port。',
  ...
  updates: {
    ...
    applyingBody: '金融助手 更新器会在自己的窗口中接管，并在完成后自动重新打开 金融助手。更新期间请不要自行重新打开 金融助手。',
