...
export const zhHant = defineLocale({
  ...
  settings: {
    ...
    appearance: {
      ...
      pet: {
        ...
        restartHint: '寵物功能需要重新啟動——目前執行的應用在此功能加入前啟動。請結束並重新開啟 金融助手，然後回到此處。',
        scaleTitle: '大小',
    ...
    gateway: {
      ...
      envOverride: '環境變數覆寫',
      intro:
        '金融助手 Desktop 預設會啟動自己的本機閘道。如果您希望此應用程式控制另一台機器或可信代理後面已執行的 金融助手 後端，請使用遠端閘道。在下方按設定檔指定各自的遠端主機。',
      appliesTo: '套用至',
      allProfiles: '全部設定檔',
      ...
      remoteTitle: '遠端閘道',
      remoteDesc: '將此桌面殼層連線至遠端 金融助手 後端。託管閘道使用 OAuth 或帳號密碼；自託管閘道也可使用工作階段 Token。',
  ...
  updates: {
    ...
    applyingBody: '金融助手 更新程式會在自己的視窗中接管，並在完成後自動重新開啟 金融助手。更新期間請勿自行重新開啟 金融助手。',
