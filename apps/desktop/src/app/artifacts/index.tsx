...
import {
  ...
  getAppActivitySession,
  launchAppActivityArtifact,
...
export function ArtifactsView({ setStatusbarItemGroup: _setStatusbarItemGroup, ...props }: ArtifactsViewProps) {
  ...
  const refreshArtifacts = useCallback(async () => {
    ...
    try {
      ...
      const [{ artifacts: conversationArtifacts, failures }, appResults] = await Promise.all([
        loadArtifactsForSessions(
          conversationPage.sessions,
      ...
      if (failures.length > 0) {
        ...
        notify({
          ...
          kind: 'warning',
          title: a.failedLoad,
          message: `Skipped ${failures.length} of ${conversationPage.sessions.length} recent sessions while indexing artifacts.`,
          detail,
          durationMs: 10_000
        })
      }

      const candidates: ArtifactRecord[] = [...conversationArtifacts]
