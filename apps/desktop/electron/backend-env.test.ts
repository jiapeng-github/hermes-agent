...
test('buildDesktopBackendEnv pins the packaged managed config when supplied', () => {
  const env = buildDesktopBackendEnv({
    hermesHome: '/Users/test/.hermes',
    managedConfigDir: '/Applications/Finance Mate.app/Contents/Resources/stocksense-managed',
  ...
  assert.equal(env.HERMES_MANAGED_DIR, '/Applications/Finance Mate.app/Contents/Resources/stocksense-managed')
})
