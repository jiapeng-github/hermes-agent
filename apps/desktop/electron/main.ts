...
async function applyGitUpdates(opts: { stopSafeBlockers?: boolean } = {}) {
  if (updateInFlight) {
...
ipcMain.handle('hermes:api', async (_event, rawRequest) => {
  ...
  if (!mutatingProfile) {
    const response = await handleHermesApiRequest(request)
...
ipcMain.handle('hermes:apps:import:select', async (_event, profile) => {
  stockSenseAccountManager.assertBackendAccess()
  const allowedProfile = stockSenseAccountManager.allowedProfile()
  const request = scopeApiRequestToAccountProfile({ path: '/api/apps/import', profile }, allowedProfile) as {
    profile?: string
  }
...
ipcMain.handle('hermes:apps:export', async (_event, appId, options = {}, profile) => {
  stockSenseAccountManager.assertBackendAccess()
  const allowedProfile = stockSenseAccountManager.allowedProfile()
  const request = scopeApiRequestToAccountProfile({ path: '/api/apps/export', profile }, allowedProfile) as {
    profile?: string
  }
