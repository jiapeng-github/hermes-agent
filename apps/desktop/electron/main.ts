...
function findGitBash() {
  ...
  const configuredGitBash = process.env.HERMES_GIT_BASH_PATH

  if (configuredGitBash && fileExists(configuredGitBash)) {
    return configuredGitBash
  }
  ...
  // Resolve custom Git for Windows installations regardless of drive letter.
  try {
    ...
    for (const root of roots) {
      for (const relative of [path.join('bin', 'bash.exe'), path.join('usr', 'bin', 'bash.exe')]) {
    ...
    // Git is not on PATH; continue with conventional install locations.
  }
